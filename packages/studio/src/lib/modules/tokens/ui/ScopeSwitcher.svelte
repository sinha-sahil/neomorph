<script lang="ts">
  ...
  import { activeHostName, hostNames, setActiveHost } from '$lib/modules/connection';
  import type { ScopeSwitcherItem } from '../types';

  let items = $derived<ScopeSwitcherItem[]>($hostNames.map((n) => ({ id: n, label: n })));
